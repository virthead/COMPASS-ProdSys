...
getRotatingFileHandler(logger, 'periodic_tasks.resend_manual_check_is_needed_jobs.log')
...
def main():
    logger.info('Getting tasks with status resend manual check is needed')
    tasks_list = Task.objects.all().filter(status='resend manual check')
    logger.info('Got list of %s tasks' % len(tasks_list))
    
    for t in tasks_list:
        logger.info('Getting jobs in status manual check is needed for task %s' % t)
        jobs_list_count = Job.objects.all().filter(task=t).filter(attempt__lt=t.max_attempts).filter(status='manual check is needed').update(status='defined',

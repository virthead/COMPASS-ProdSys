...
getRotatingFileHandler(logger, 'periodic_tasks.check_cta_dump_status.log')

logger.info('Starting %s' % __file__)

pid = str(os.getpid())
logger.info('pid: %s' % pid)
if check_process('check_cta_dump_status.py', pid):
...
def restart_transfer(logger, task, run_number, chunk_number):
    try:
        j_update = Job.objects.filter(task=task, run_number=run_number, chunk_number_merging_evntdmp=chunk_number).update(status_castor_evntdmp='ready', date_updated=timezone.now())
...
def check_files_on_cta():
    ...
    tasks_list = Job.objects.filter(task__tapes_backend='cta').filter(status_castor_evntdmp='sent').values_list('task_id', 'task__path', 'task__soft', 'task__prodslt', 'task__phastver', 'task__type').distinct()
    logger.info('Got list of %s prods: %s' % (len(tasks_list), tasks_list))
    logger.info('Check details in the corresponding periodic_tasks.check_cta_dump_status_taskid.log')
    
    for t in tasks_list:
        logger_task = logging.getLogger('periodic_tasks_logger')
        getRotatingFileHandler(logger_task, 'periodic_tasks.check_cta_dump_status_%s.log' % t[0])
        ...
        if result.succeeded:
            ...
            for c in chunks_list:
                ...
                found = False
                test = '%(path)sevtdump%(prodSlt)s-%(runNumber)s.raw' % {'path': path, 'runNumber': c[1], 'prodSlt': t[3]}
                    ...
                    if r['name'] == test:
                        ...
                        if result1.succeeded:
                            ...
                            if responses[0]['on_tape']:
                                ...
                                try:
                                    j_update = Job.objects.filter(task=t[0], run_number=c[1], chunk_number_merging_evntdmp=c[2]).update(status_castor_evntdmp='finished', date_updated=timezone.now())

...
def check_files_on_castor():
    ...
    for t in tasks_list:
        ...
        getRotatingFileHandler(logger_task, 'periodic_tasks.check_castor_mdst_status_%s.log' % t[0])
        ...
        if t[5] == 'MC reconstruction':
            cmd = 'nsls -l %(castorHome)smc_prod/CERN/%(Year)s/%(Period)s/%(prodSoft)s/mcreco/mDST/' % {'castorHome': settings.CASTOR_HOME, 'prodPath': t[1], 'prodSoft': t[2], 'Year': t[6], 'Period': t[7]}
        else:

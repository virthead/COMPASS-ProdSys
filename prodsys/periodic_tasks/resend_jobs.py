...
def main():
    logger.info('Getting tasks with status resend')
    tasks_list = Task.objects.all().filter(status='resend')
    logger.info('Got list of %s tasks' % len(tasks_list))
    
    for t in tasks_list:
        logger.info('Getting jobs in status failed for task %s' % t)
        jobs_list_count = Job.objects.all().filter(task=t).filter(attempt__lt=t.max_attempts).update(status='defined',

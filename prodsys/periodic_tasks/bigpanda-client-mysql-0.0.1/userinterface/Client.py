...
# get job ouput size
def getJobOutputSize(ids,srvID=None):
    """Get job output size
       ...
       returns:
           status code
                 0: communication succeeded to the panda server 
                 255: communication failure
           the list of file sizes (or Nones for non-existing PandaIDs)
    """     
    # serialize
    strIDs = pickle.dumps(ids)
    # instantiate curl
    curl = _Curl()
    # execute
    url = _getURL('URL',srvID) + '/getOutputSize'
    data = {'ids':strIDs}
    status,output = curl.post(url,data)
    try:
        return status,pickle.loads(output)
        ...
        type, value, traceBack = sys.exc_info()
        errStr = "ERROR getJobOutputSize : %s %s" % (type,value)
        print errStr
        return EC_Failed,output+'\n'+errStr

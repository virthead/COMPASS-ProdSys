...
getRotatingFileHandler(logger, 'periodic_tasks.check_cta_mdst_status.log')

logger.info('Starting %s' % __file__)

pid = str(os.getpid())
logger.info('pid: %s' % pid)
if check_process('check_cta_mdst_status.py', pid):
...
def check_files_on_cta():
    ...
    tasks_list = Job.objects.filter(task__tapes_backend='cta').filter(status_castor_mdst='sent').values_list('task_id', 'task__path', 'task__soft', 'task__prodslt', 'task__phastver', 'task__type', 'task__year', 'task__period').distinct()
    ...
    for t in tasks_list:
        ...
        getRotatingFileHandler(logger_task, 'periodic_tasks.check_cta_mdst_status_%s.log' % t[0])
        ...
        if t[5] == 'MC reconstruction':
            path = '%(ctaHome)smc_prod/CERN/%(Year)s/%(Period)s/%(prodSoft)s/mcreco/mDST/' % {'ctaHome': settings.CTA_HOME, 'prodPath': t[1], 'prodSoft': t[2], 'Year': t[6], 'Period': t[7]}
        else:
            path = '%(ctaHome)s%(prodPath)s%(oracleDst)s%(prodSoft)s/mDST/' % {'ctaHome': settings.CTA_HOME, 'prodPath': t[1], 'prodSoft': t[2], 'oracleDst': oracle_dst}
        ...
        if result.succeeded:
            ...
            for c in chunks_list:
                ...
                found = False
                test = '%(path)smDST-%(runNumber)s-%(prodSlt)s-%(phastVer)s.root' % {'path': path, 'runNumber': c[1], 'prodSlt': t[3], 'phastVer': t[4]}

...
getRotatingFileHandler(logger, 'periodic_tasks.check_castor_mcgen_status.log')
...
if check_process('check_castor_mcgen_status.py', pid):
...
def restart_transfer(logger, task, run_number, chunk_number):
    try:
        j_update = Job.objects.filter(task=task, run_number=run_number, chunk_number=chunk_number).update(status_castor_mcgen='ready', date_updated=today)
...
def check_files_on_castor():
    ...
    logger.info('Check details in the corresponding periodic_tasks.check_castor_mcgen_status_taskid.log')
    
    for t in tasks_list:
        logger_task = logging.getLogger('periodic_tasks_logger')
        getRotatingFileHandler(logger_task, 'periodic_tasks.check_castor_mcgen_status_%s.log' % t[0])
        ...
        cmd = 'nsls -l %(castorHome)smc_prod/CERN/%(Year)s/%(Period)s/%(prodSoft)s/mcgen/' % {'castorHome': settings.CASTOR_HOME, 'Year': t[1], 'Period': t[2], 'prodSoft': t[3]}
                ...
                for r in reader:
                    if r['name'] == test:
                        ...
                        if r['permissions'][0] == 'm':
                            ...
                            try:
                                j_update = Job.objects.filter(task=t[0], run_number=c[1], chunk_number=c[2]).update(status_castor_mcgen='finished', date_updated=today)

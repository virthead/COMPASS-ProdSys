        ...
        for r in runs_list:
            ...
            for l in copy_list:
                ...
                if result.succeeded:
                    ...
                    jobs_update = Job.objects.filter(task=t).filter(run_number=r).filter(chunk_number=chunk).update(status_castor_mcgen='sent', attempt_castor_mcgen=1, date_updated=today)
                    logger.info(result)
                else:
                    ...
                    if result.find('No such file or directory') != -1:
                        ...
                        jobs_list = Job.objects.filter(task=t).filter(run_number=r).filter(chunk_number=chunk).update(status='failed', status_castor_mcgen=None, date_updated=today)

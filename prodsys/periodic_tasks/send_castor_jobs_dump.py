    ...
    for t in tasks_list:
        ...
        for r in runs_list:
            ...
            for c in merged_chunks_list:
                f_from = 'fts-transfer-submit -s %(ftsServer)s -o %(eosHomeRoot)s%(eosCompassHome)s%(prodPath)s%(prodSoft)s/mergedDump/slot%(prodSlt)s/evtdump%(prodSlt)s-%(runNumber)s.raw' % {'prodPath': t.path, 'prodSoft': t.soft, 'runNumber': r, 'prodSlt': t.prodslt, 'phastVer': t.phastver, 'ftsServer': settings.FTS_SERVER, 'eosHomeRoot':settings.EOS_HOME_ROOT, 'eosCompassHome': settings.EOS_COMPASS_HOME}
                if format(c, '03d') != '000':

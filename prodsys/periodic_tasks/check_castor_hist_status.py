...
def check_files_on_castor():
    ...
    for t in tasks_list:
        logger.info('Getting histos with castor histos status sent for task id %s' % t[0])
                    ...
                    if r['name'].find(test) != -1:
                        ...
                        if r['permissions'][0] == 'm':
                            ...
                            try:
                                ...
                                logger.info('Job status_castor_histos changed to finished for task %s run number %s chunk number %s' % (t[0], c[1], c[2]))
                            except:
                            ...
                            if r['size'] == '0':
                                ...
                                try:
                                    ...
                                    logger.info('Job status_castor_histos changed to ready for task %s run number %s chunk number %s' % (t[0], c[1], c[2]))
                                except:
                                    logger.error('Failed to update jobs for task %s run number %s chunk number %s' % (t[0], c[1], c[2]))

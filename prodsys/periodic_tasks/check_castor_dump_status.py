...
if check_process('check_castor_dump_status.py', pid):
...
def check_files_on_castor():
    ...
    for t in tasks_list:
        ...
        cmd = 'nsls -l %(castorHome)s%(prodPath)s%(oracleDst)s%(prodSoft)s/mergedDump/slot%(prodSlt)s/' % {'castorHome': settings.CASTOR_HOME, 'prodPath': t[1], 'prodSoft': t[2], 'prodSlt': t[3], 'oracleDst': oracle_dst}

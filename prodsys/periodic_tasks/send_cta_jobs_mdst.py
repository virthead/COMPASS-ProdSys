...
getRotatingFileHandler(logger, 'periodic_tasks.send_cta_jobs_mdst.log')
...
if check_process("send_cta_jobs_mdst.py", pid):
    ...
    for t in tasks_list:
        ...
        for r in runs_list:
            ...
            for c in merged_chunks_list:
                ...
                f_from = 'fts-transfer-submit -s %(ftsServer)s -o %(eosHomeRoot)s%(eosHome)s%(mc)s%(prodPath)s%(prodSoft)s/mDST/mDST-%(runNumber)s-%(prodSlt)s-%(phastVer)s.root' % {'prodPath': t.path, 'prodSoft': t.soft, 'runNumber': r, 'prodSlt': t.prodslt, 'phastVer': t.phastver, 'ftsServer': settings.FTS_SERVER, 'eosHomeRoot':settings.EOS_HOME_ROOT, 'eosHome': settings.EOS_HOME, 'mc': mc}
                if format(c, '03d') != '000':
                    f_from = f_from + '.' + format(c, '03d')
                
                if t.type == 'MC reconstruction':
                    f_to = '%(tapesHomeRoot)smc_prod/CERN/%(Year)s/%(Period)s/%(prodSoft)s/mcreco/mDST/mDST-%(runNumber)s-%(prodSlt)s-%(phastVer)s.root' % {'tapesHomeRoot': tapesHomeRoot, 'prodPath': t.path, 'prodSoft': t.soft, 'runNumber': r, 'prodSlt': t.prodslt, 'phastVer': t.phastver, 'Year': t.year, 'Period': t.period}
                else:
                    f_to = '%(tapesHomeRoot)s%(prodPath)s%(oracleDst)s%(prodSoft)s/mDST/mDST-%(runNumber)s-%(prodSlt)s-%(phastVer)s.root' % {'tapesHomeRoot': tapesHomeRoot, 'prodPath': t.path, 'prodSoft': t.soft, 'runNumber': r, 'prodSlt': t.prodslt, 'phastVer': t.phastver, 'oracleDst': oracle_dst}
                if format(c, '03d') != '000':
            ...
            for l in copy_list:
                ...
                if result.succeeded:
                    ...
                    jobs_update = Job.objects.filter(task=t).filter(run_number=r).filter(chunk_number_merging_mdst=chunk).update(status_castor_mdst='sent', attempt_castor_mdst=1, date_updated=timezone.now())
                    logger.info(result)
                else:
                    ...
                    if result.find('No such file or directory') != -1:
                        ...
                        jobs_list = Job.objects.filter(task=t).filter(run_number=r).update(status_merging_mdst='ready', chunk_number_merging_mdst=-1, status_x_check='no', status_castor_mdst=None, date_updated=timezone.now())

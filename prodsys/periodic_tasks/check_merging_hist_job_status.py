                ...
                if j_check.jobstatus == 'finished' or j_check.jobstatus == 'failed' or j_check.jobstatus == 'closed':
                    ...
                    if j_check.jobstatus == 'failed' or j_check.jobstatus == 'closed':
                        ...
                        if j_check.piloterrorcode == 1235 or j_check.piloterrorcode == 1237 or j_check.piloterrorcode == 1242:
                            ...
                            jobs_list_update = Job.objects.filter(panda_id_merging_histos=j_check.pandaid).update(status_merging_histos='manual check is needed', date_updated=timezone.now())
                        else:
                            jobs_list_update = Job.objects.filter(panda_id_merging_histos=j_check.pandaid).update(status_merging_histos='failed', date_updated=timezone.now())
                    elif j_check.jobstatus == 'finished':
                        jobs_list_update = Job.objects.filter(panda_id_merging_histos=j_check.pandaid).update(status_merging_histos=j_check.jobstatus, status_castor_histos='ready', date_updated=timezone.now())
